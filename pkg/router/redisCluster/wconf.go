...
paramLoop:
	...
	if err != nil || len(value) == 0 {
		return WriteConfig(s.Writer, consumerID, nil)
	}
	w.Stop()

	return WriteConfig(s.Writer, consumerID, value)
}

func (r *Router) checkConfigChanged(configKey string, updateAt int64) bool {
	timestamp, err := rediscluster.Int64(r.redisCluster.Do("HGET", configKey, "timestamp"))
	if err != nil {
		return false
	}
...
func (r *Router) getConfig(configKey string) ([]interface{}, error) {
	return rediscluster.Values(r.redisCluster.Do("HGETALL", configKey))
}

func WriteConfig(local *RedSHandle.WriterHandle, consumerID string, reply map[string]*wconfig.ConfigResponse) (err error) {
	if len(reply) == 0 {
		...
		logrus.Infof("配置变化通知consumerID：%s， 配置%s", consumerID, c.ConfigID)
